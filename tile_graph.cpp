...
TileGraph::TileGraph(int w, int h)
{
	...
	// NOTE: This could probably be also made with constructor
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			tiles[x + (y*w)].SetPos(x, y);
            tiles[x + (y*w)].SetType(0); // logic to calculate t to be made
		}
	}
...
void TileGraph::Setup(int w, int h)
{
	...
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			tiles[x + (y*w)].SetPos(x, y);
            tiles[x + (y*w)].SetType(0); // logic to calculate t to be made in mapgenerator
		}
	}
